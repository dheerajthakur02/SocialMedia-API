...
export const userLogin = async(req,res) =>{
   try {
          ...
          if(!existingUser){
             return res.status(404).json({message:"User not existed in database, please register"});
          }
        ...
        const user ={
         ...
         return res.status(200).cookie("token",token, {maxAge:1*60*60*1000, httpsOnly:true, sameSite:"Strict"}).json({message:`Login successfully !!Welcome ${user._id}`, user})
   } catch (error) {
      return res.status(500).json({message:"Internal server error", error: error.message})
   }
}
...
export const removeUser = async (req,res) =>{
   try {
       ...
       if(admin && admin.role != "admin"){
         return res.status(404).json({message:"You are not admin, so you are not authorised to remove the user"});
       }
